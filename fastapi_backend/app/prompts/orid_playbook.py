...
def _narration_bucket_first_paragraph_rule(bucket: str) -> str:
    ...
    return (
        "- 第一段（你已經做到）：**盡量**嵌入【學生草稿摘錄】或 JSON 的 student_anchor_quote 裡的片段（用引號標出 4～15 字即可），讓人覺得你真的讀過原文；"
...
def _progressive_narration_rules() -> str:
    return (
        ...
        "- **第三段短引導（RASF-Anchor）**：第三段只保留一個下一步；**優先**使用 JSON 的 rasf.draft_next_step，"
        "說明在學生 anchor 句前/後加什麼，不要用「故事的主角是……」從零重寫。"
        "若 JSON 的 suggestions 是問句，保留；再接一個接在 anchor 上的半句填空（保留＿＿＿）。"
...
- 第三段（試著補一句）：只給**一個**在學生原文前/後的下一步；**優先** rasf.draft_next_step + 一個問句。禁止「故事的主角是……」通用開頭；禁止「我們一步一步來」。
