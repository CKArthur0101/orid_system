...
def _progressive_narration_rules() -> str:
    return (
        ...
        "- **例外（O 段／故事覆蓋）**：若 JSON 的 missing 或 suggestions **明顯在談**「故事摘要／大事件／還沒寫到的一段／掃摘要」，"
        "代表主點是**對照教材補齊情節**，即使第一段也稱讚了順序或事件清楚，也**不要**把第二、三段改寫成只剩轉折詞、句型或「小升級」式微調；"
        "應保留「先對照摘要，把還缺的那一大段用一句話寫進來」的意圖，只把語氣接得溫和、與第一段不打架即可。\n"
        "- **第三段可跨大步（課堂約 40 分鐘寫四格）**：在仍是引導、不代寫整篇的前提下，第三段可用「先……；再……；最後……」排 **2～4 個短任務**（同一小段內），"
        "必要時再接「例如：」**1～2 句**示範起頭（分號連）；總量比先前允許略多，但仍要口語、能一口氣讀完。"
    )
...
def _stage_segment_coaching(stage: str) -> str:
    ...
    if s == "O":
        return (
            ...
            "- 【O 段／JSON 對齊】若結構化 JSON 裡 missing、suggestions 在談**摘要缺段、補大事件**，"
...
- 第三段（試試看）：學生常**一節課約 40 分鐘要寫完四格**，第三段請給**較完整的一串引導**（仍不要整篇範文）：可用「先……；再……；最後……」排 **2～4 個短任務**，讓他們一輪能多往前寫；再接「例如：」**1～2 句**示範起頭（分號連）。若 JSON 裡有 example，請融入這一段；不要只留抽象指令。
...
- 學生約 **40 分鐘要寫完四格**，回覆可**一次給較完整的一串小步**（像口語 checklist：用分號或先再最後，**2～4 步**），仍保持短段落、**不要代寫整段**。
