...
def _narration_bucket_first_paragraph_rule(bucket: str) -> str:
    if bucket in (BUCKET_LIKELY_GIBBERISH, BUCKET_TOO_SHORT, BUCKET_EMPTY):
        return (
            ...
            "第三段（可以這樣修改）若 JSON 已點到書裡具體事件，請**照寫進去**；不要改成只剩「一步一步」空架。"
        )
    return (
...
def _progressive_narration_rules() -> str:
    return (
        ...
        "  1. 優先 JSON 的 rasf.draft_next_step（說明在 anchor 句前/後加什麼；要改成明確格子落點：O 觀察格／R 感受格／I 體會格／D 行動格）\n"
...
def _stage_segment_coaching(stage: str) -> str:
    ...
    if s == "O":
        return (
            ...
            "敘述時只說「故事裡還有重要情節沒寫到／請回想哪一幕」，**不要**把具體事件名寫成學生可抄的答案；"
            "三段內文**不要**對學生說「去看摘要／掃摘要／對照故事摘要」。\n"
            "- 【RQ3】第三段請明確請他**回到 O 觀察格，補角色、事件或情節**，不要給可整段複製的範文。\n"
        )
    if s == "R":
        return (
...
def format_feedback_narration_playbook(
    *,
    ...
    return f"""
你是寫作回饋同伴。下面是一份「結構化回饋」JSON。請改寫成給**國小五年級**讀的訊息，**剛好三段**，每段最多 1～2 個短句。標題必須是（字可微調，但三段結構與順序不變）：
