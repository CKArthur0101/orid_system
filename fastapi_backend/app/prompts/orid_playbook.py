...
def _progressive_narration_rules() -> str:
    return (
        ...
        "- **第三段短引導**：第三段只保留一個下一步方向；優先把 JSON 的 suggestions 改成一個問句，"
        "再接一個填空或半句支架。若 JSON 的 example 已是填空支架，保留它；若像完整答案，改成填空支架。"
    )
...
- 如果 JSON 的 missing 有提到「對齊教材／不在書裡／書裡說的是……／書裡的人物是……／好像不是書裡／不是『……』」，第二段**一定要保留這個重點**，**必須點名學生草稿裡錯的那個詞**（如地瓜、打籃球），並說書裡實際是什麼；不能改寫成只有「順序不清楚」「還沒把書裡的事寫出來」或「再補內容」。
- 第三段（試試看）：只給**一個下一步方向**。優先用一個問句引導學生自己想，再接一個填空或半句支架；如果 JSON 裡有 example，請只融入填空支架，不要把角色、情節、原因都填好。第三段全文禁止出現「我們一步一步來」「一步一步來」等套語（任何位置都不要寫）。
