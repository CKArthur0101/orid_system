...
def _progressive_narration_rules() -> str:
    return (
        ...
        "  1. 優先 JSON 的 rasf.draft_next_step（說明在 anchor 句前/後加什麼；可加「請回到你這一格」）\n"
...
def _stage_segment_coaching(stage: str) -> str:
    ...
    if s == "O":
        return (
            ...
            "- 【RQ3】第三段請明確請他**回到 O 格**補／改一句，不要給可整段複製的範文。\n"
        )
    if s == "R":
        return (
    ...
    if s == "D":
        return (
            ...
            "- 【RQ3】第三段請他回到 D 格把行動寫具體一點，不要代寫完整計畫。\n"
        )
    return ""
...
- 第三段（可以這樣修改）：只給**一個**下一步；**優先** rasf.draft_next_step + 一個問句；可加「請回到你這一格」。禁止「故事的主角是……」通用開頭；禁止「我們一步一步來」；禁止完整範文代寫。
