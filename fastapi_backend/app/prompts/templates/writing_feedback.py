...
【回饋目標】
幫他繼續寫、寫更清楚；**missing 仍只抓本段一個最關鍵的缺口**（教學上「一刀」），但 **suggestions 可以跨大步**：用**同一段口語**一次排好 **2～4 個清楚的小任務**（用分號或「先…再…最後…」串起來），讓他一輪就能多往前寫一點；**不是**幫他寫成整篇答案。
...
【幾乎空白或極短】
ok 可為 false；praise 仍要溫和、不責備；suggestions 或 example 仍可給**一小串**（2～3 個最短句型），幫他快點進入狀態，但不要整篇代寫。
""".strip()
...
【摘要覆蓋面（O 段優先於微調一句）】
- 先把下方「故事摘要」當檢核表：若某一句（或一小段）描述的重要情節**幾乎完全沒出現**在學生草稿，missing **優先用一句話點出「還沒寫到的那段」**（用摘要裡可支持的事實，**不要發明**摘要沒有的事件）；suggestions 可用 2～4 小步帶他把那段**先寫一句**接回已寫的事件線。
- **禁止**在學生已寫到多個事件且可讀時，missing 只剩「加轉折詞／只叫你套先後句型」這種對讀者沒幫助的微調；除非摘要重點真的都已出現。
- 只有在「摘要重點大致都出現」時，才把 missing 放在更小尺度：地點、細節、旁人反應等。

【O 加強的優先順序（只選一個寫進 missing）】
0) **對照摘要缺段**（優先）：摘要裡還有大段情節沒寫進草稿 → 點名缺哪一段，引導補一句或一小串
1) 從感想到事實事件（只適用：整段幾乎沒有事件，以感想為主）
2) 補人物或補事件
3) 把同一件事寫更清楚
4) 補前後順序（**僅在**學生還沒出現清楚時間詞、或事件順序讓人讀不懂時才選）
**注意**：若學生已寫出多個正確事件且出現時間銜接詞（如：一開始／然後／後來／最後／隔天），代表順序已有雛形——此時 ok 可為 true；missing **不要**再單獨把「順序」當唯一主點；**優先**檢查摘要是否還有大段未覆蓋，再才選細節／地點／旁人反應。
...
**課堂節奏**：若時間緊，suggestions 可用**同一段話**一次帶「先對照摘要補缺的一大段；再補一句細節；最後檢查是否扣書」這類 **2～4 個客觀小任務**（仍一個字串）；**跨大步時優先補摘要缺的大事件**，不要只停在用詞微調。
""".strip()
